...
def main():
    ...
    tmpdir = "%s/banxcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and banxcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "banxcoind"]) is not None:
            print("%sWARNING!%s There is already a banxcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BANXCOIND" not in os.environ:
        os.environ["BANXCOIND"] = build_dir + '/src/banxcoind' + exeext
        os.environ["BANXCOINCLI"] = build_dir + '/src/banxcoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `banxcoin-cli help` (`rpc_interface.txt`).
